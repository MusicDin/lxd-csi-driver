...
func (c *controllerServer) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	if contentSource != nil {
		...
		case *csi.VolumeContentSource_Volume:
			...
			if err != nil {
				return nil, status.Errorf(codes.Internal, "CreateVolume: Failed to retrieve source volume: %v", err)
			}
			...
			if sourceVolSizeBytes > sizeBytes {
				return nil, status.Errorf(codes.InvalidArgument, "CreateVolume: Volume size %d is lower then the source volume size %d", sizeBytes, sourceVolSizeBytes)
			}
		default:
